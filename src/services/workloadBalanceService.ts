...
import JudgeReassignmentTrail, {
  IJudgeReassignmentTrail,
} from "../models/JudgeReassignmentTrail";
...
export class WorkloadBalanceService {
  static async analyzeWorkload(
    ...
    const judgeWorkloads = await Promise.all(
      judges.map(async (judge) =>
        this.buildJudgeWorkloadDetail(judge, warningThreshold),
      ),
    );
    ...
    const underloaded = sortedWorkloads.filter(
      (j) => j.workloadPercentage < 50,
    );
    ...
    const imbalanceRatio =
      avgWorkload > 0 ? (maxWorkload - minWorkload) / avgWorkload : 0;
  ...
  private static async buildJudgeWorkloadDetail(
    ...
    const activeCaseList = await CaseModel.find({
      judgeId: judge._id,
      status: {
        $in: [CaseStatus.PENDING, CaseStatus.PROCESSING, CaseStatus.URGENT],
      },
    }).sort({ acceptedDate: 1 });

    const reassignableCases = activeCaseList.filter(
      (c) => DEFAULT_REASSIGNABLE_STAGES.includes(c.stage) && !c.trialStartDate,
    );
    ...
    const workloadPercentage =
      (activeCaseList.length / judge.maxCaseLoad) * 100;
    ...
    return {
      ...
      overloadCount: Math.max(
        ...
        activeCaseList.length -
          Math.floor((warningThreshold / 100) * judge.maxCaseLoad),
      ),
      reassignableCases,
      casesBySpecialty,
    };
      ...
      const availableJudges = analysis.allJudges
        .filter(
          (j) =>
            ...
            j.judge.circuitCourtId.toString() ===
              overloadedJudge.judge.circuitCourtId.toString(),
        )
        .map((j) => ({
          judge: j.judge,
  ...
  private static async enhanceCaseWithJudgeRecommendations(
    ...
    const preferredSpecialties = this.getPreferredSpecialtiesForCase(
      caseItem.caseType as CaseType,
    );
    ...
    const recommendedJudges = allJudges
      .filter(
        (j) =>
          ...
          j.judge.circuitCourtId.toString() ===
            fromJudge.circuitCourtId.toString(),
      )
      .map((j) => {
        ...
        return {
          ...
          capacityAvailable,
          overallScore:
            Math.round((specialtyMatchScore + workloadScore) * 10) / 10,
        };
      })
  ...
  private static getPreferredSpecialtiesForCase(
    caseType: CaseType,
  ): JudgeSpecialty[] {
  ...
  private static getMatchedSpecialtiesForCase(
    ...
    if (!caseItem) return [];
    const preferred = this.getPreferredSpecialtiesForCase(
      caseItem.caseType as CaseType,
    );
    ...
    try {
      ...
      if (caseDoc.judgeId?.toString() !== input.fromJudgeId.toString()) {
        ...
        return {
          ...
          fromJudgeId: input.fromJudgeId,
          toJudgeId: input.toJudgeId,
          message: "案件当前承办人与指定的原法官不一致",
        };
      }

      if (
        caseDoc.trialStartDate ||
        !DEFAULT_REASSIGNABLE_STAGES.includes(caseDoc.stage)
      ) {
        await session.abortTransaction();
        return {
          success: false,
          ...
          fromJudgeId: input.fromJudgeId,
          toJudgeId: input.toJudgeId,
          message: "该案件已进入实质审理阶段，无法改派",
        };
      }

      const fromJudge = await Judge.findById(input.fromJudgeId).session(
        session,
      );
      ...
      if (
        toJudge.circuitCourtId.toString() !==
        fromJudge.circuitCourtId.toString()
      ) {
        await session.abortTransaction();
        return {
      ...
      const matchedSpecialties = this.getMatchedSpecialtiesForCase(
        caseDoc,
        toJudge,
      );
  ...
  static async getReassignmentHistory(
    ...
    if (filter.reassignmentType)
      query.reassignmentType = filter.reassignmentType;
  ...
  private static buildAnalysisFilter(query: any): {
    filter: WorkloadAnalysisFilter;
    threshold: number | undefined;
  } {
    const { courtId, specialty, warningThreshold } = query || {};
    const filter: WorkloadAnalysisFilter = {};
    const courtObjectId = toOptionalObjectId(courtId);
    if (courtObjectId) filter.circuitCourtId = courtObjectId;
    if (
      specialty &&
    ...
    return { filter, threshold: toOptionalNumber(warningThreshold) };
  }

  static async handleAnalyzeWorkload(query: any) {
    const { filter, threshold } = this.buildAnalysisFilter(query);
    return this.analyzeWorkload(filter, threshold);
  }

  static async handleReassignmentSuggestions(query: any) {
    const { filter, threshold } = this.buildAnalysisFilter(query);
    const suggestions = await this.generateReassignmentSuggestions(
      filter,
      threshold,
    );
    return {
  ...
  static async handleJudgeWorkloadBoard(query: any) {
    const { courtId, warningThreshold } = query || {};
    const circuitCourtId = toOptionalObjectId(courtId);
    const threshold = toOptionalNumber(warningThreshold);
    return this.getJudgeWorkloadBoard(circuitCourtId, threshold);
  }

  private static REASSIGNMENT_TYPES: Array<
    "load_balance" | "specialty_adjustment" | "manual" | "other"
  > = ["load_balance", "specialty_adjustment", "manual", "other"];

  private static parseReassignmentInput(raw: any): ReassignmentExecutionInput {
    const {
      caseId,
      fromJudgeId,
      toJudgeId,
    ...
    if (
      ...
      throw new ServiceValidationError("请填写完整的改派信息");
    }

    if (!this.REASSIGNMENT_TYPES.includes(reassignmentType)) {
      throw new ServiceValidationError("无效的改派类型");
    }

    return {
      caseId: toObjectId(caseId),
      fromJudgeId: toObjectId(fromJudgeId),
      toJudgeId: toObjectId(toJudgeId),
      reassignmentReason,
      reassignmentType:
        reassignmentType as ReassignmentExecutionInput["reassignmentType"],
      operatorId,
      operatorName,
      remark,
    };
  ...
  static async handleExecuteReassignment(
    body: any,
  ): Promise<ReassignmentResult> {
    const input = this.parseReassignmentInput(body);
    return this.executeReassignment(input);
  }

  static async handleBatchExecuteReassignment(body: any): Promise<{
    results: ReassignmentResult[];
    successCount: number;
    failCount: number;
    totalCount: number;
  }> {
    const { reassignments } = body || {};

    if (!Array.isArray(reassignments) || reassignments.length === 0) {
      throw new ServiceValidationError("请提供改派列表");
    }
    ...
    const results = await this.batchExecuteReassignment(inputs);
  ...
  static async handleReassignmentHistory(query: any) {
    const {
      caseId,
      fromJudgeId,
      toJudgeId,
    ...
    const cId = toOptionalObjectId(caseId);
    if (cId) filter.caseId = cId;
    const fId = toOptionalObjectId(fromJudgeId);
    if (fId) filter.fromJudgeId = fId;
    const tId = toOptionalObjectId(toJudgeId);
    if (tId) filter.toJudgeId = tId;
    if (reassignmentType) filter.reassignmentType = reassignmentType;
    const sDate = toOptionalDate(startDate);
    if (sDate) filter.startDate = sDate;
    const eDate = toOptionalDate(endDate);
    if (eDate) filter.endDate = eDate;

    return this.getReassignmentHistory(filter);
  }
}
